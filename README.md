...
| `hyprland.lua` | **Window and shell decoration** — rounding 18, `rounding_power` 2.05, borders, gaps, blur, window opacity, the shell-surface `layer_rule`s and every animation. Loaded from the active theme by `default/hypr/omarchy.lua`, so it applies on `omarchy theme set` with no relogin. The two themes carry **identical, deliberately duplicated** copies: they are meant to stand alone. **Only while the theme is symlinked from a local checkout** — Omarchy refuses to stage `.lua` from a theme installed by git URL, since it runs in the compositor. |
