...
- **Window rounding/gaps/borders/blur** (BUILD.md §5) belong in `~/.config/hypr/`,
  ...
  overrides window opacity to `0.97 0.875` (re-matched onto browsers directly too,
...
- **Shell-surface translucency lives in the theme, per section.** A theme-shipped
  ...
  washed out.
- **Focused windows at 0.97, unfocused at 0.875, so the blur renders through both.**
  ...
  sets `0.97 0.875` — focused isn't fully opaque either, so it reads as the same
  ...
  `browser.lua` has run) and pins those to `0.97 0.875` too — same glass as
