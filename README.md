...
| `backgrounds/` | Wallpapers — macOS stock (Big Sur → Sequoia) plus macOS-styled community art; 26 dark / 28 light that Omarchy will actually use. Its picker filters on `jpg/jpeg/png/gif/bmp/webp`, so the six `.heic` files here are silently skipped — they cycle nowhere and are only taking up space. The `00-` prefix on `00-umeda_wallpaper_desktop*.png` is what makes it each theme's default: Omarchy has no default-background key and simply takes the sort-first file when switching into a theme. |
