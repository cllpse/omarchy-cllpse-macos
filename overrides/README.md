...
- **`diff` shadows `/usr/bin/diff`.** Its guard is `command -v git`, which is
  true on every machine this targets, so unlike the others it is always active.
  ...
  does too. But an interactive `diff a b` is `git diff a b`, which is a
  different tool with different arguments and output. It exists so diffs go
  through the `hunk` pager from `git/pager.conf`; if you want diffutils back,
  delete the line from `bash/shell.sh` rather than from `~/.bashrc`, or the next
  apply puts it back.
...
hypr/looknfeel-decoration.lua rounding 18 / rounding_power 2.2 / border_part_of_window true / blur / border_size 2 / gaps 12,24 / groupbar off / window opacity 0.99 0.875 (the focused 1% is ~60% of the blur cost, kept deliberately; Figma fully opaque) / 3x animations, floor 1 / layer_rule blur on shell surfaces
