...
The script is [`keyd.sh`](keyd.sh). It is runnable on its own and is also
...
## 1. if command -v keyd >/dev/null 2>&1; then
...
## 2. if [[ -e /etc/keyd/default.conf ]] && ! head -1 /etc/keyd/default.conf
...
## 3. skip "overrides/keyd/default.conf does not parse — left /etc alone. Ru
...
## 4. keyd reload >/dev/null 2>&1 || sudo systemctl restart keyd >/dev/null
...
## 6. if [[ -x ~/.local/bin/cllpse-figma-keyd ]] && command -v keyd >/dev/nu
