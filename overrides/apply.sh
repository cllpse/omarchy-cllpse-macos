...
# keybind-allowlist.conf is seeded from that scan once; it's yours from then
# on -- delete a line to have the next apply unbind it. Every apply re-scans
# the live set fresh and regenerates keybind-unbinds.lua from the current
# allowlist, so an Omarchy update that adds new default binds is pruned too,
...
if [[ -e $ALLOWLIST ]]; then
  ...
  say "seeding keybind-allowlist.conf from every bind currently in effect"
  tmp=$(mktemp)
  lua "$HERE/hypr/keybind-scan.lua" dump >"$tmp"
  if [[ ! -s $tmp ]]; then
    rm -f "$tmp"
    printf 'error: keybind scan produced nothing — leaving keybind-allowlist.conf unseeded\n' >&2
  else
    {
      echo "# Every Hyprland keybind currently in effect, one per line as"
      echo "# \"keys<TAB>description\". Delete a line to have the next apply run"
      echo "# unbind it. apply.sh never rewrites this file once it exists --"
      echo "# delete it yourself and re-apply to reseed from scratch."
      echo "#"
      cat "$tmp"
    } >"$ALLOWLIST"
    rm -f "$tmp"
...
if [[ -e $ALLOWLIST ]]; then
  say "keybind-unbinds.lua: diffing the live bind set against keybind-allowlist.conf"
  {
