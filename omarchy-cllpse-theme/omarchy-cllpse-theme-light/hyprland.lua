-- Window and shell decoration for cllpse-macos.
--
-- Loaded by Omarchy from the ACTIVE theme (default/hypr/omarchy.lua requires
-- `omarchy.current.theme.hyprland`), so it applies on `omarchy theme set` with
-- no relogin -- verified, not assumed.
--
-- SELF-CONTAINED ON PURPOSE. This theme is meant to stand alone, so nothing
-- here is shared with the other cllpse-macos theme: the two files are
-- deliberate duplicates and must be edited together. `cp -r` in
-- omarchy-theme-set:269 recreates symlinks rather than following them, so a
-- shared file linked from both would arrive dangling anyway.
--
-- CAVEAT, and it is a real one: Omarchy refuses to stage any .lua from a theme
-- installed from a git URL (omarchy-theme-set:142) -- it executes in the
-- compositor. Everything in this file therefore only applies while the theme is
-- SYMLINKED from a local checkout, which is what overrides/apply.sh does. Ship
-- this theme by URL and the decoration silently disappears; the colours, icons
-- and shell surfaces still arrive.
--
-- What is NOT here: the presentation-popup width rule, which has to load after
-- Omarchy's own window rules and so stays in overrides/hypr/.
...
-- own Electron build) is a colour-critical design tool --
...
-- is the lowercase "figma-desktop", not the "Figma" the AppImage's own
-- .desktop declares as StartupWMClass. Strip the tag and set opacity explicitly, same
